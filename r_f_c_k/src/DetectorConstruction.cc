...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  */
  ...
  G4ThreeVector pos1 = G4ThreeVector(0, -1*m, 0);

  // Shape 1
  // int scale =100;
  // G4double shape1_a = scale*3.78*mm;
  // G4double shape1_b = scale*3.78*mm;
  // G4double shape1_hz = scale/20*18.9*mm;
  ...
  G4RotationMatrix* rotation_1 = new G4RotationMatrix();
    rotation_1->rotateX(fA_X * deg);
    rotation_1->rotateY(fA_Y * deg);
    rotation_1->rotateZ(fA_Z * deg);

  fDetector = new G4PVPlacement(rotation_1,G4ThreeVector(fP_X,fP_Y,fP_Z), logicShape1, "Shape1", logicWorld, false, 0, checkOverlaps);
...
void DetectorConstruction::SetRot_X(G4double val_X)
{
    if (!fDetector) {
        G4cerr << "Detector has not yet been constructed." << G4endl;
        return;
    ...
    rotation_2->rotateX(fA_X );
    rotation_2->rotateY(fA_Y );
    rotation_2->rotateZ(fA_Z );




    fDetector->SetRotation(rotation_2);
...
void DetectorConstruction::SetRot_Y(G4double val_Y)
{
    if (!fDetector) {
        G4cerr << "Detector has not yet been constructed." << G4endl;
        return;
...
void DetectorConstruction::SetRot_Z(G4double val_Z)
{
    if (!fDetector) {
        G4cerr << "Detector has not yet been constructed." << G4endl;
        return;
...
void DetectorConstruction::DefineCommands()
{
    ...
    auto& setRotX
        = fMessenger->DeclareMethodWithUnit("setRotX","deg",
                                            &DetectorConstruction::SetRot_X,
                                            "Set position of the detector.");
    setPositionY.SetParameterName("fA_X", true);

    auto& setRotY
        = fMessenger->DeclareMethodWithUnit("setRotY","deg",
                                            &DetectorConstruction::SetRot_Y,
                                            "Set position of the detector.");
    setPositionY.SetParameterName("fA_Y", true);

    auto& setRotZ
        = fMessenger->DeclareMethodWithUnit("setRotZ","deg",
                                            &DetectorConstruction::SetRot_Z,
                                            "Set position of the detector.");
    setPositionY.SetParameterName("fA_Z", true);
